...
lpi %>% 
  group_by(PrimaryKey) %>% 
  add_count(TopCanopy, name = 'Vascular_Intercepts') %>% 
  mutate(Invasive = if_else(length))
...
```{r Number of Invasive and Introduced derived per plot}

# we will spin up some nice maps here using the full templates. 
ggplot(inv_richness, aes(color = Prop_Int)) +
  geom_sf()

ggplot(inv_richness, aes(color = Prop_Inv)) +
  geom_sf()
...
```{r Total Proportion of Invasive Species Cover per Plot}
